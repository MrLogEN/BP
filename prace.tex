...
%\usepackage[top=25mm,bottom=25mm,right=25mm,left=30mm,head=12.5mm,foot=12.5mm]{geome%try}
...
\def\NazevPrace{Aplikace pro monitorování kvality vnitřního prostředí}

%%% Jméno autora
%%% Author's name - First name Surname
\def\AutorPrace{Vilém Charwot}

%%% Rok odevzdání a měsíc (slovně)
%%% Year of submission and month (verbally) - month YYYY
\def\DatumOdevzdani{červen 2026}

%%% Vedoucí práce: Jméno a příjmení s~tituly
%%% Supervisor: First name and surname with titles
\def\Vedouci{Ing. Dušan Chlapek, Ph.D.}
...
{
	\pagestyle{plain}
	\include{zarizeni_analyza}
}

{
	\pagestyle{plain}
	\include{aplikace_analyza}
}
...
\include{appendices/legislativa}

\include{appendices/archimate-repo}

\include{appendices/implementace-repo}

\include{appendices/nasazeni}

\include{appendices/usecases}

\include{appendices/pozadavky}

\include{appendices/prilohy_korespondence}

...
\usepackage{rotating}       % otočené (landscape) obrázky a tabulky / rotated (landscape) figures and tables
\usepackage{dirtree}        % sazba adresářových stromů / directory tree typesetting
\usepackage{pgf-umlsd}      % UML sekvenční diagramy / UML sequence diagrams
...
\usepackage{threeparttable} % tabulky s poznámkami přímo pod tabulkou / table notes below table
\usepackage{threeparttablex} % rozšíření threeparttable pro longtable / extends threeparttable for longtable

\usepackage{pdfpages}

% Source - https://tex.stackexchange.com/a/2442
% Posted by frabjous, modified by community. See post 'Timeline' for change history
...
% Zalamování a prevence zalamování 
  \hyphenation{JSON}
  \hyphenation{GeoJSON}
  \hyphenation{HTTP}
  \hyphenation{HTTPS}
...
\input{custom-commands/pripad-uziti}
\input{custom-commands/funkcni-tabulka}
\input{custom-commands/poz-tabulka}
\input{custom-commands/usecase-actor}

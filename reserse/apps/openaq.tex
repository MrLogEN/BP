\FloatBarrier
\subsection{Aplikace OpenAQ}

Druhou aplikací je americká aplikace \textit{OpenAQ} z New Yorku. Tato aplikace se, jak je uvedeno v dokumentaci API, zaměřuje především na měření kvality vnějšího ovzduší. Podporované veličiny jsou uvedeny v tabulce č. \ref{tab:open-aq-veliciny} \parencite{API}. Z té je zřejmé, že aplikace vůbec nemyslí na měření veličin akustického či světelného komfortu. 

Data v aplikaci OpenAQ jsou poskytována dvěma způsoby. 
Prvním z nich je přístup přes REST API, který umožňuje získat data ve formátu JSON. Tento přístup je vhodný pro programatické zpracování dat. Pro používání API jsou dostupné sady vývojových nástrojů (SDK) pro skriptovací jazyky Python a R. \parencite{API}  
...
Za pomocí API byly identifikovány senzory, které jsou na platformě OpenAQ používány. Jejich shrnutí je uvedeno v tabulce č. \ref{tab:openaq-supported-sensors}. V tabulce jsou uvedeny pouze veličiny, které jsou relevantní pro IEQ a byly v rámci rešerše autorem identifikovány. Je nutné poznamenat, že aplikace není závislá na konkrétním hardwaru. Uvedené senzory jsou pouze příklady, kdy autoři dat publikovali, jaký konkrétní hardware používají. To umožňuje uživatelům dat lépe porozumět, s jakou spolehlivostí a přesností data interpretovat. \parencite{API}

Nezávislost na konkrétním hardwaru je výhodná z pohledu udržitelnosti, neboť není nutné implementovat podporu pro každý nový senzor. Břemeno napojení se tím přenáší na publikujícího uživatele, což sice může zvýšit vstupní bariéru, ale zároveň zvyšuje flexibilitu a škálovatelnost platformy.

{
  ...
  \begin{longtable}{L{3.3cm}@{\hspace{8pt}}*{8}{c}@{\hspace{8pt}}*{3}{c}@{\hspace{8pt}}*{2}{c}@{\hspace{8pt}}*{2}{c}}
    ...
    \label{tab:openaq-supported-sensors}\\
    \toprule
...
Webové rozhraní aplikace je pro zobrazování dat na světové mapě podobné jako u aplikace Sensor.Community (viz obrázek č. \ref{obr:openaq-map}). Uživatelské rozhraní je pro zobrazování dat přístupné bez předchozí registrace. Uživatelé mohou zobrazení výsledků filtrovat podle: 
\begin{itemize}
...
Tyto filtry jsou zobrazeny na obrázku č. \ref{obr:openaq-map-filters}. Po kliknutí na konkrétní senzor se zobrazí stručné informace o daném senzoru (viz obrázek č. \ref{obr:openaq-map-sensor}). Po kliknutí na odkaz pro zobrazení detailů senzoru se zobrazí detailní charakteristika senzoru (viz obrázek č. \ref{obr:openaq-sensor-detail-characteristics}), naměřené hodnoty konkrétního senzoru (viz obrázek č. \ref{obr:openaq-sensor-detail-reading}), vzory v naměřených hodnotách konkrétního senzoru (viz obrázek č. \ref{obr:openaq-sensor-detail-pattern}) a možnost stáhnout CSV export dat za vybrané období pro konkrétní senzor (viz obrázek č. \ref{obr:openaq-sensor-detail-download}).

\begin{figure}[htbp!]\centering
\caption{Mapa s daty o kvalitě ovzduší na webovém rozhraní aplikace OpenAQ}
\label{obr:openaq-map}
...
\label{obr:openaq-sensor-detail-characteristics}
...
\begin{figure}[htbp!]\centering
\caption{Naměřené hodnoty konkrétního senzoru v aplikaci OpenAQ}
\label{obr:openaq-sensor-detail-reading}
...
\label{obr:openaq-sensor-detail-pattern}
...
\label{obr:openaq-sensor-detail-download}

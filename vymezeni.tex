...
V textu se objevují jednotky \textit{ppm}, \textit{ppb}, \textit{$\mu\text{g}*m^{-3}$}, \textit{°C}, \textit{K}, \textit{\%}, \textit{dB} a další. Každá z těchto jednotek má své specifické použití a význam v kontextu IAQ a IEQ.
...
V IAQ se pro CO\textsubscript{2} nebo eCO\textsubscript{2}\footnote{odhadovaná koncentrace CO\textsubscript{2} senzorem} se často používá jednotka \textit{ppm}. Pro plyny jako NO\textsubscript{2}, SO\textsubscript{2} a CO se v předpisech běžně používají jednotky \textit{$\mu\text{g}*m^{-3}$}. Ale je možné použít i jednotku \textit{ppm} nebo \textit{ppb}. Pro VOC/TVOC je standadní uvádět měřené hodnoty pomocí \textit{ppb}, pro laboratorní měření ja nejčastěji používanou jednotkou \textit{$\mu\text{g}*m^{-3}$}. Pro PM\textsubscript{2.5} a PM\textsubscript{10} se běžně používají jednotky \textit{$\mu\text{g}*m^{-3}$}.

V rámci tepelného komfortu se pro měření teploty běžně používají jednotky \textit{°C}, ale pro vědecké účely je vhodnější používat jednotku \textit{K}. Pro relativní vlhkost se používá bezezrozměrná jednotka \textit{\%}, která udává, do jaké míry je vzduch nasycen vodou. Pro rychlost proudění vzduchu jsou běžně používanou jednotkou \textit{m/s} (metry za sekundu). 

Vizuální komfort se měří pomocí jednotky \textit{lx} (lux), která udává intenzitu osvětlení. Pro barevnou teplotu se používá jednotka \textit{K} (Kelvin). Oslňování se často měří pomocí jednotky \textit{cd/m\textsuperscript{2}} (kandela na metr čtvereční).

Akustický komfort se měří pomocí jednotky \textit{dB} (decibel) nebo \textit{dB(A)} (A-vážený decibel), která zohledňuje citlivost lidského ucha na různé frekvence zvuku.

Jednotky ppm, ppb se dají převádět na $\mu\text{g}*m^{-3}$, kde je nutné znát podmínky měření (T, p) a molární hmotnost látky. Zda tento převod dává smysl, závistí na konkrétní látce.

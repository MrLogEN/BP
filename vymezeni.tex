...
První aplikací je německá aplikace \textit{sensor.community} ze Stuttgartu. Aplikace je založena na komunitě dobrovolníků, kteří instalují senzory pro měření různých veličin a sdílejí je ve veřejné databázi. 

Zaměření aplikace je především na měření kvality vnějšího prostředá, ale má možnost měřit i prostředí vnější 

Sledované veličiny aplikací jsou uvedeny v tabulce č. \ref{tab:sensor-community-veliciny}.
...
\begin{table}[hp!]
  ...
  \begin{tabular}{p{2.8cm} p{5.8cm} p{1.7cm} >{\raggedright\arraybackslash}p{4.6cm}}
    ...
    \midrule
    \multirow{11}{*}{IAQ} & PM$_{1.0}$ (hmotnostní koncentrace) & $\mu$g/m$^3$ & ---\\
...
- srovnávací tabulka sdílených dat pomocí API jednotlivých aplikací (tabulka)

- srovávací tabulka dat sdílených pomocí dumpů jednotlivých aplikací (tabulka)

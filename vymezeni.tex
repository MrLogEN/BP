...
\caption{Shrnutí jednotek pro vybrané veličiny IEQ (včetně alternativ používaných ve zdrojích a senzorech)}\label{tab:ieq-jednotky}
...
Vyhláška č.~43/2025~Sb. \parencite{info@wolterskluwer.czVyhlaska432025} &
...
Metodický návod ZD~25/2013 \parencite{MetodickyNavodZD252013} &
Metodika měření &
...
O\textsubscript{3} & AOT40\footnotemark\allowbreak{} ($\mu\text{g}\,m^{-3}\cdot$h) & 18000 & 18000 & — & Ochrana vegetace; 5letý průměr \\
...
\renewcommand{\arraystretch}{1.05}
\scriptsize
\caption{Limitní hodnoty pro vnitřní prostředí pobytových místností dle vyhlášky č.~43/2025~Sb. (ČR)}\label{tab:indoor_limity_cr}
\begin{tabularx}{\textwidth}{@{}
...
>{\raggedright\arraybackslash}p{0.23\textwidth}
  ...
  X@{}}
\toprule
\textbf{Parametr} & \textbf{Doba (jedn.)} & \textbf{ČR} & \textbf{EU}\footnotemark & \textbf{WHO} & \textbf{Pozn.} \\
\midrule

$LA_{eq,8h}$\footnotemark (pracoviště, expoziční limit) & 8 h (dB) & 85 & — & — &
...
Attribution Share-alike & CC BY-SA 4.0 & ODbL 1.0 & Attribution s Copyleft\footnotemark přístupem.  \\
\bottomrule
\end{tabularx}
\end{center}
...
\footnotesize 
\end{table}
\footnotetext{Copyleft je druh licence, která umožňuje uživatelům svobodně používat, upravovat, kopírovat, sdílet a redistribuovat dílo, ale s podmínkou, že všechny odvozené práce musí být také licencovány pod stejnout licencí. \parencite{Copyleft2026}}
...
Na rozhraní můžeme také vidět barevnou škálu pro právě vybranou veličinu (viz č. 1 na obrázeku č. \ref{obr02:sensor-community-filter}). Po vybrání veličiny je možné vybrat jeden či více zobrazovaných senzorů a získat tak konkrétní naměřenou hodnotu (viz obrázke č. \ref{obr03:sensor-community-sensor-reading}). 
...
%tag-sensor-community-supported-sensors
\begin{table}[htbp!]
  \centering
  \footnotesize
  \setlength{\tabcolsep}{2pt}
  ...
  \begin{tabular}{*{1}{l}@{\hspace{12pt}}*{8}{c}@{\hspace{12pt}}*{3}{c}@{\hspace{12pt}}*{2}{c}@{\hspace{12pt}}*{2}{c}}
    \toprule
...
\hline
Radiation SBM-19 \footnote{Radiace} & & & & & & & & & & & & & & & \\ %radiation
\hline
...
SPS30 & \cmark & \cmark & & & & & & & & & & & & & \\
  ...
  \footnotesize \textit{Zdroj:} \parencite{APISensorCommunity, SensorCommunityDevices}
\end{table}
...
\begin{table}[htbp!]
  ...
  \begin{tabular}{p{2.7cm}p{2.1cm}p{2.48cm}p{2.2cm}p{2.48cm}p{2.48cm}}
    \toprule
    \textbf{Aplikace} & \textbf{Komunikač-ní model} & \textbf{Formát archivu} & \textbf{Formát API}  & \textbf{Protokol API} & \textbf{Úložiště (kategorie)}\\
    \midrule
    \multirow{1}{*}{Sensor.Community} & Push &  CSV, ZIP & JSON & HTTP/HTTPS & InfluxDB (časové řady)\\
    \midrule
    \multirow{1}{*}{OpenAQ} & --- & --- & --- & ---\\
    \midrule
    \multirow{1}{*}{OpenSense} & --- & --- & --- & ---\\
    \midrule
    \multirow{1}{*}{IndoorCO2Map} & --- & --- & --- & ---\\
    \bottomrule
  \end{tabular}
...
\begin{table}[htbp!]
  ...
  \begin{tabular}{lm{1.3cm}|cccc}
    ...
    \hline
    \multirow{7}{*}{IAQ}               & PM\textsubscript{2.5}               & \cmark & x & x & x \\
                                       & PM\textsubscript{10}                & \cmark & x & x & x \\
                                       & O\textsubscript{3}  & \xmark & x & x & x \\
                                       & NO\textsubscript{2} & \cmark & x & x & x \\
                                       & SO\textsubscript{2} & \xmark & x & x & x \\
                                       & CO                  & \xmark & x & x & x \\
                                       & CO\textsubscript{2} & \cmark & x & x & x \\
                                       & TVOC                & \xmark & x & x & x \\
    \hline
    \multirow{3}{*}{Tepelný komfort}   & $T$ & \cmark & x & x & x \\
                                       & RH & \cmark & x & x & x \\
                                       & $v$ & \xmark & x & x & x \\
    \hline
    \multirow{2}{*}{Vizuální komfort}  & $E$ & \xmark & x & x & x \\
                                       & CCT & \xmark & x & x & x \\
    \hline
    \multirow{2}{*}{Akustický komfort} & $L_A$ & \cmark & x & x & x \\
                                       & $L$ & \xmark & x & x & x \\
  \bottomrule
  \end{tabular}
